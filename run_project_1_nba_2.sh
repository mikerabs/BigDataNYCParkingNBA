...
# Validate inputs
if ! [[ "$PROGRAM_NUM" =~ ^[1-2]$ ]]; then
  echo "Error: First argument must be a number between 1 and 2."
  exit 1
fi
...
hadoop fs -rm -r /user/johndavis/output_shot_counts
...
hadoop jar $HADOOP_HOME/share/hadoop/tools/lib/hadoop-streaming-*.jar \
  ...
  -input /user/johndavis/input/shot_logs.csv \
  -output /user/johndavis/output_shot_counts \
  -mapper "./mapper_nba_${PROGRAM_NUM}.py" \
  -reducer "./reducer_nba_${PROGRAM_NUM}.py" \
  -file "mapper_nba_${PROGRAM_NUM}.py" \
  -file "reducer_nba_${PROGRAM_NUM}.py" \
  -numReduceTasks 1
...
hadoop fs -cat /user/johndavis/output_shot_counts/part-00000 | \
  awk -F'\t' "NF==$NUM_FIELDS" | \
